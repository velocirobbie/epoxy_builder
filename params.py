...
class Parameterise(object):
    def __init__(self, crystal, vdw_defs, new_connections):
        ...
        crystal.angle_coeffs = self.match_angles(
                crystal.angle_coeffs, new_connections['angles'])
        crystal.torsion_coeffs = self.match_torsions(
                crystal.dihedral_coeffs, new_connections['dihedrals'])
    ...
    def match_angles(self, angle_coeffs, new_angles):
        ...
        def search_angles(angle_data, atoms, angle_coeffs):
            ...
            for j in range(len(angle_data['k'])):
                ...
                if flag1 or flag2:
                    found += 1
                    #try: angle_coeffs[label]; raise Exception
                    #except KeyError: pass 
                    angle_coeffs[label] = {}
                    angle_coeffs[label][1] = angle_data['k'][j]
                    angle_coeffs[label][2] = angle_data['r'][j]
            return angle_coeffs, found

        angle_data = self.angle_data
        questionable_substitutions = 0
        for label in new_angles:
            atoms = new_angles[label]
    ...
    def match_torsions(self, torsion_coeffs, new_torsions):
        def add_coeff(torsion_data, torsion_coeffs, j):
            torsion_coeffs[label] = {}
            torsion_coeffs[label][1] = torsion_data['k1'][j]
            torsion_coeffs[label][2] = torsion_data['k2'][j]
            torsion_coeffs[label][3] = torsion_data['k3'][j]
            torsion_coeffs[label][4] = torsion_data['k4'][j]
        ...
        questionable = 0
        for label in new_torsions:
            atoms = new_torsions[label]
            a1,a2,a3,a4 = atoms 
            ...
            if found == 0:
                ...
                torsion_coeffs,found = search_torsions(torsion_data, atoms, 
                                                   torsion_coeffs)
            if found == 0:
                if atoms[3] == 47: atoms[3] = 3
                if atoms[0] == 47: atoms[0] = 3
                torsion_coeffs,found = search_torsions(torsion_data, atoms, 
                                                   torsion_coeffs)
                if not found: 
                    if atoms[3] == 3: atoms[3] = 47
                    if atoms[0] == 3: atoms[0] = 47
            ...
            if found != 1:
                raise ValueError('Torsion',atoms,
                                 'found ',found,' entries')
