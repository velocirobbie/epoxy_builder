...
class ReadLammpsData(object):
    ...
    def analyse(self,line,datafile):
        ...
        elif len(line) >= 2 and line[1] == 'Coeffs':
    ...
    def read_atoms(self,datafile):
        ...
        self.ids = np.zeros((self.Natoms),dtype=int)
        self.charges = np.zeros(self.Natoms)
        self.molecules = np.zeros(self.Natoms,dtype=int)
        self.atom_labels = np.zeros(self.Natoms, dtype=int)
        for i in range(self.Natoms):
            line = self.read(datafile)
            self.ids[i] = int(line[0])
            self.coords[i] = line[4:7]
            self.molecules[i] = int(line[1])
            self.atom_labels[i] = int(line[2])
    ...
    def read_bonds(self,datafile):
        self.bonds = np.zeros((self.Nbonds,2),dtype=int)
    ...
    def read_angles(self,datafile):
        self.angles = np.zeros((self.Nangles,3),dtype=int)
    ...
    def read_dihedrals(self,datafile):
        self.dihedrals = np.zeros((self.Ndihedrals,4),dtype=int)
    ...
    def read_impropers(self,datafile):
        self.impropers = np.zeros((self.Nimpropers,4),dtype=int)
